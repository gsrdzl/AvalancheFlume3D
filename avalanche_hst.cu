...
int main ()
{
  ...
  cp_in = fgets (row, sizeof (row), stdin);
  sscanf (row, "%lf", &(sys_par.side.y));    /* Flume width. */
  fprintf (fp_log, "%s", cp_in);
  cp_in = fgets (row, sizeof (row), stdin);
  ...
  cudaMalloc ((void **) &cell_vec_dev,  ntotmax*sizeof (long));
  cudaMalloc ((void **) &nocup_vec_dev, n3max*sizeof (int));
  ...
  fp_xyz = fopen ("snapshots_hst.xyz", "w");
#else
  ...
  /* Writes initial configurations to xyz file. */
  fp_xyz = fopen ("snapshots_hst.xyz", "a");
#endif
  ...
  /* ========================== RUN ======================== */
#if FIRST_TIME
  fp_vmean = fopen ("mean_velocity_hst.csv", "w");
  fp_vel   = fopen ("velocities_hst.xyz",    "w");
  fp_for   = fopen ("forces_hst.dat",        "w");
  fp_xcm   = fopen ("xcm_hst.csv",           "w");
#else
  fp_vmean = fopen ("mean_velocity_hst.csv", "a");
  fp_vel   = fopen ("velocities_hst.xyz",    "a");
  fp_for   = fopen ("forces_hst.dat",        "a");
  fp_xcm   = fopen ("xcm_hst.csv",           "a");
#endif  
  if (!aval_start)
        ...
        for (ii = 0; ii < nn; ii++)
        {
          if (print_flag)
          {
            ...
            fprintf (fp_vel, "%lf %lf %lf %lf %lf %ld\n", pp_par_hst[ii].rr.x, pp_par_hst[ii].rr.y,
                     pp_par_hst[ii].rr.z, pp_par_hst[ii].vv.x, pp_par_hst[ii].ww.y, ii);
          }
          fprintf (fp_conf_out,
...
/* ======================== AVALANCHE TIME ======================== */
  while (sys_par.time < 2.0)
  {
    ...
    if (vmean < 0.1*sys_par.v0 && sys_par.time > 0.2)
      break;
  }
